...
- Per shot the chain is: decode to float, **override the container's colour tags** with the encoding the mode names and confirm range, the **input transform** to ACEScct, then the shot's **CLF**.
- **The source encoding has two modes, set in Settings** (`docs/COLOR_AND_FORMAT.md` section 1, decided 2026-09-12). In **Camera log** mode, which is the expectation, each clip's own metadata names the log its camera shot and the input transform is per shot. In **Studio standard** mode one Settings value names it for the whole batch, and that standard is **DaVinci Wide Gamut / DaVinci Intermediate**. Everything downstream of the input transform is identical either way, which is why this is one string and not two pipelines.
- **Camera mode's bill is a name to resolve per shot.** The metadata carries what a human typed in Resolve and OCIO wants one of its own colour space names, which is OQ-34, reopened. A written name that names no colour space, or names more than one, blocks the row (QC-046, QC-047) rather than falling back to a default: a plate rendered through the wrong input transform looks entirely normal.
