...
- Consolidated media: one file or image sequence per timeline clip, **ProRes 4444 in camera native log**, with the encoding **written into the clip's metadata by the shooter** (`docs/COLOR_AND_FORMAT.md` sections 1 and 2, OQ-44). S-Log3, C-Log3 and BM Film are what is expected today and more may be added. A studio standard encoding on every file is the alternative the tool can be switched to (FR-12, FR-15, OQ-39). Extra frames beyond the timeline In/Out (handles are already in the timeline range; the extra frames only exist so Out can be extended).
...
- Per shot the chain is: decode to float, **override the container's colour tags** with the encoding the mode names and confirm range, then the shot's **CLF**. **The tool applies no input transform** (answered 2026-09-12, OQ-37): the colourist builds the CLF from the source encoding itself, so the CLF is the whole transform from what arrived to linear ACEScg, and ACEScct is no longer part of the chain.
- **The source encoding has two modes, set in Settings** (`docs/COLOR_AND_FORMAT.md` section 1, decided 2026-09-12). In **Camera log** mode, which is the expectation, each clip's own metadata names the log its camera shot. In **Studio standard** mode one Settings value names it for the whole batch, and that standard is **DaVinci Wide Gamut / DaVinci Intermediate**. The colourist starts the CLF from whichever it is, so the two modes differ in one string and not in what the tool does.
...
- **Where that transform is applied is a correctness rule, not a diagram detail.** It is applied on the **aux still**, which is delivered ungraded and never gets the CLF, and on any row with no CLF at all. It is **not** applied on a graded plate or reference, because the CLF already starts at the source encoding and applying both converts twice, which looks like a grade decision and passes every check. Whether a given session's CLF really does contain it is OQ-46, recorded rather than assumed.
...
M4.5 Colour pipeline, core only: OCIO wired in, the final EDL's In/Out and CDL read and matched per row, the CLF matched and loaded, the chain composed as one GroupTransform, the viewing LUT. Reopens M3's render for the plate/view split. No Qt, testable headless. It was built with an input transform ahead of the CLF, which OQ-37 removed on 2026-09-12; taking it back out is M4.6.
M4.6 Camera log mode: the source encoding read per shot from the clip metadata, the input transform removed from the plate and view chains, the source-to-ACEScg conversion kept for the aux still alone, QC-046 and QC-047.
