...
| OQ-19 | QC-026 severity now that the timeline rate is authoritative. Shooters conform every clip to 24 in Resolve, so a media file whose own header still says otherwise is stale camera metadata rather than a retime hazard: the timeline is what it is played at, frame counts come from the file, and nothing computes with the stated rate. Blocking such a row as an error may be too strict. QC_RULES currently makes QC-026 an error that auto-skips the row | Keep as an error, since QC_RULES says so, and revisit against a real turnover | no |
