...
| QC-023 | error | row | Source resolution is not 3840x2160. Enabling "allow non-4k" downgrades it to a warning rather than silencing it. The render letterboxes or pillarboxes a source of another shape onto the target with black, after the colour chain (`resize.fit_inside`, chunk G), and never stretches or crops. Aux stills and BTS are not asked: they are delivered at their own size |
| QC-024 | retired | row | Retired 2026-09-23: letterboxing is a render step, and QC-023 already says the source is another size |
