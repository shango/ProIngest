...
| QC-039 | error | row | The CLF appears to contain a display rendering: probing it shows an output that is bounded or otherwise not scene linear ACEScg. The result would be a display referred file claiming to be linear, which comps wrong and looks completely normal until someone tries to work on it (COLOR_AND_FORMAT section 1). **The probe's margin is narrower than it was, and for one camera it is gone**: it compares the CLF's white against a fixed floor, and white is worth 222 out of ACEScct but 14.7 out of C-Log3, so a dark grade on a C-Log3 CLF lands where a display rendering lands. Measured, written up with the numbers and a ratio probe that does separate them, as **OQ-47**. Unchanged until that is answered, because the failure direction is a valid grade refused rather than a display referred plate delivered |
