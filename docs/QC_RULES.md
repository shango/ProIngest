...
| QC-038 | error where the row has an aux still, else warning | row | The studio standard source encoding named in Settings has no OpenColorIO equivalent, so the aux still's conversion cannot be built. **Studio mode only**; camera mode's equivalents are QC-046 and QC-047, which differ in who has to fix them: this one is a Settings value the editor typed, those are metadata a shooter wrote. Scoped to the aux still on 2026-09-12 with them, since OQ-37 removed the input transform from every other chain. Batch wide in practice rather than per row, because the encoding is one constant for every file (COLOR_AND_FORMAT section 1), but it is reported on the row because the row is what cannot render |
