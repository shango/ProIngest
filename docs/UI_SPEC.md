...
- **A section with nothing behind it yet is listed and disabled**, and its page says what it is waiting on. Same rule as the toolbar in section 1. **Output is the only one left** (reference quality and EXR compression level): those are applied inside a worker process, so a setting has to travel on the render job rather than be read from the page.
